...
import { CalendarDays, ChevronDown, ChevronLeft, ChevronRight, Circle, Diamond, GripVertical, Link2, Plus, RotateCcw, Settings, Sun, Moon, Monitor, X, CircleHelp, Ellipsis, Globe, HardDrive, Download, Upload, ExternalLink, Copy, Pencil, Trash2 } from "lucide-react";
...
import { relatedDepths, hasCycle, deleteProjectContent, type Task, type TaskType, type Dependency, type Port } from "@/lib/schedule";
...
import {backupFilename, createBackup, CURRENT_DATA_VERSION, migratePersistedState, parseBackup, type PersistedState} from "@/lib/persistence";
...
import {deleteWorkType} from "@/lib/work-types";
...
function initialProjects(): Project[] {
  return [
    { id: "p1", name: "研究项目 A", color: projectPalette[0] },
    { id: "p2", name: "个人计划", color: projectPalette[1] },
  ];
}
...
function initialTasks(): Task[] {
  ...
  return [
    { id: "t1", title: "资料收集", projectIds: ["p1"], order: {}, type: "论文", start: iso(addDays(today, -5)), end: iso(addDays(today, 2)), status: "已完成" },
    { id: "t2", title: "方案整理", projectIds: ["p1"], order: {}, type: "整理", start: iso(addDays(today, -1)), end: iso(addDays(today, 8)), status: "进行中" },
    { id: "t3", title: "阶段验证", projectIds: ["p1"], order: {}, type: "算法/仿真", start: iso(addDays(today, 7)), end: iso(addDays(today, 17)), status: "未开始" },
    { id: "t4", title: "初稿整理", projectIds: ["p1"], order: {}, type: "论文", start: iso(addDays(today, 14)), end: iso(addDays(today, 26)), status: "未开始" },
    { id: "t5", title: "环境准备", projectIds: ["p2"], order: {}, type: "工程", start: iso(addDays(today, 1)), end: iso(addDays(today, 6)), status: "进行中" },
    { id: "t6", title: "阶段复盘", projectIds: ["p2"], order: {}, type: "idea与思考", start: iso(addDays(today, 11)), end: iso(addDays(today, 11)), status: "未开始", milestone: true },
  ];
}

const emptyTask = (projectId = "p1"): Task => ({ id: "", title: "", memo: "", projectIds: [projectId], order: {}, type: "算法/仿真", start: iso(new Date()), end: iso(addDays(new Date(), 3)), status: "未开始" });
...
type Schedule = { projects: Project[]; tasks: Task[]; edges: Dependency[]; inbox:InboxItem[] };
type PersonalDefaults=Partial<{
  ...
  tasks:{defaultDays:number;defaultTaskType:TaskType;hoverSpeed:"fast"|"medium"|"slow";completedMode:"fade"|"normal"|"hide";barHeight:number;barRadius:number;barFill:number;completedOpacity:number};
...
function TimelineApp() {
  ...
  const [history, setHistory] = useState(() => historyOf<Schedule>({ projects: initialProjects(), tasks: initialTasks(), edges: initialEdges(), inbox:[] }));
  const { projects, tasks, edges, inbox } = history.present;
  ...
  const todoDeleteBackup=useRef<null|{kind:"inbox";item:InboxItem;index:number}|{kind:"task";task:Task;index:number;edges:Dependency[]}>(null);
  ...
  type ProjectDeleteBackup={project:Project;index:number;tasks:Array<{task:Task;index:number}>;edges:Dependency[];collapsed:boolean};
  ...
  useEffect(() => {
    try {
      let restored: Schedule = { projects: initialProjects(), tasks: initialTasks(), edges: initialEdges(), inbox:[] };
      const unified = localStorage.getItem("research-gantt-v2");
      if (unified) {
        applyPersistedState(migratePersistedState(JSON.parse(unified)));
      } else {
  ...
  }, [projects, tasks, edges, inbox, inboxKind, inboxSort, inboxCollapsed, collapsedProjects, defaultDays, autoConnectionSides, handleWidth, handleGap, edgeLevel, arrowLevel, personalDefaults, theme, primaryColor, tablePalettes, colors, customColorPresets, appearance, editorMode, settingsWidth, timelineRowHeight, dayWidth, weekStart, defaultTaskType, hoverSpeed, completedMode, insertionDelay, wordingStyle, inboxCompleted, inboxSize, ready]);
  useEffect(() => {
  ...
  const projectGroups=useMemo(()=>projects.map(project=>{
    const items=(rowPreview??tasks).filter(task=>task.projectIds.includes(project.id)&&(completedMode!=="hide"||task.status!=="已完成")).sort((a,b)=>(a.order[project.id]??0)-(b.order[project.id]??0));
  ...
  }),[projects,tasks,rowPreview,completedMode]);
  ...
  const rowHeight=(row:VisibleRow)=>row.kind==="insert"?(row.phase==="hint"?timelineRowHeight/4:timelineRowHeight):row.kind==="add"?layoutTokens.addRowHeight:timelineRowHeight;
  ...
  const movePan = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const next = panViewport(element.scrollLeft,event.clientX-active.lastX,windowScrollLimit(cellCount*cellWidth,element.clientWidth,frozenWidth),cellWidth);
  ...
  const saveOnEnter = (event:React.KeyboardEvent<HTMLElement>,save:()=>void) => {
    ...
    // Portalled choices and keyboard-operated controls retain their own Enter action.
    if(!event.currentTarget.contains(target)||target.closest('button,textarea,select,[role="combobox"],[role="listbox"],[role="radio"],[role="checkbox"],[contenteditable="true"]'))return;
  ...
  const saveTask = () => {
    ...
    const normalized = { ...editing, title: editing.title.trim() || t("未命名"), end: editing.end < editing.start ? editing.start : editing.end };
  ...
  const confirmProjectDelete=()=>{
    ...
    projectDeleteBackup.current={project,index:projects.findIndex(item=>item.id===project.id),tasks:tasks.flatMap((task,index)=>task.projectIds.includes(project.id)?[{task,index}]:[]),edges:cascade?edges.filter(edge=>result.edges.every(item=>item.id!==edge.id)):[],collapsed:collapsedProjects.has(project.id)};
  ...
  const undoProjectDelete=()=>{
    ...
    changeSchedule(current=>{
      ...
      return {...current,projects,tasks,edges:[...current.edges,...backup.edges.filter(edge=>!edgeIds.has(edge.id))]};
    });
  ...
  const moveDrag = (event: React.PointerEvent, task: Task) => {
    ...
    trackInsertion(event.clientY);
    const distance = event.clientX - drag.startX;
    const cells = Math.round(distance / cellWidth);
    setDrag({ ...drag, previewCells: cells, previewY:event.clientY-drag.startY, moved: drag.moved || Math.abs(distance) > 4 || Math.abs(event.clientY-drag.startY)>4 });
  };
  ...
  const addTaskToProject = (projectId: string, start=iso(zoom==="month"?addDays(anchor,Math.floor(scrollOffset/cellWidth)):new Date()), targetRowId?:string, exact=false) => {
    ...
    setEditing({ ...emptyTask(projectId), ...range, type:defaultTaskType, status:statusForRange(range.start,range.end), rowId:targetRowId, id: crypto.randomUUID(), order: { [projectId]:targetRowId?(tasks.find(task=>rowId(task)===targetRowId)?.order[projectId]??tasks.length):tasks.length } }); setEditError("");
    setEditingEdges([]);
  };
  ...
  const reset = () => { const freshProjects = initialProjects(); const freshTasks = initialTasks(); changeSchedule(current => ({ ...current, projects: freshProjects, tasks: freshTasks, edges: initialEdges() })); setSelectedTask(null); setCollapsedProjects(new Set()); };
  ...
  const connections = useMemo(() => {
    ...
    return edges.flatMap(edge => {
      ...
      return [...pairs].map(([key,[i,j]]) => {
        ...
        return { key: edge.id+"-"+key, edge:displayedEdge, from, to, path: smoothRoute([from,...route,to],layoutTokens.cornerRadius,arrowSize), conflicts: portDate(edge.target,target) < portDate(edge.source,source) };
      });
    });
  }, [visibleRows, tasks, edges, drag, resize, anchor, cellWidth, stepDays, autoConnectionSides, scrollOffset, barTop, barBottom, rowTops, arrowSize]);
  ...
  useEffect(()=>{
    ...
    const move=(event:PointerEvent)=>{
      ...
      current.to={x:event.clientX-canvas.left,y:event.clientY-canvas.top}; current.target=null;
      let nearest=20;
      for(const element of Array.from(timeline.current.querySelectorAll<HTMLElement>("[data-link-task]"))){
        ...
        const {port,point}=linkPortAt(element,task,side,event.clientX);
      ...
      if(current.target){
        ...
        const points=candidates.map(el=>linkPortAt(el,task,target.side,event.clientX).point);
    ...
    const finish=(event:PointerEvent)=>{
      ...
      if(current.active&&current.target){
        const edge:Dependency={id:current.edgeId??crypto.randomUUID(),source:current.movingEnd==="source"?current.target:current.source,target:current.movingEnd==="source"?current.source:current.target};
  ...
  const previewPath = useMemo(()=>{
    ...
    return smoothRoute([from,...routeAroundTasks(outsideFrom,outsideTo,obstacles),to],layoutTokens.cornerRadius,arrowSize);
  },[linkPreview,visibleRows,anchor,cellWidth,stepDays,barTop,barBottom,arrowSize]);
  ...
  const renderTask=(task:Task)=>{
    ...
    if(g.placeholder&&!activeDrag)return <button className="task-body absolute overflow-hidden" aria-label={t(g.placeholder==="future"?"{0}，晚于当前视图":"{0}，早于当前视图", t(task.title))} title={t(task.title)+" · "+task.start+"—"+task.end} aria-pressed={selectedTask===task.id} style={{...taskStyle,zIndex:task.milestone?45:20}} onClick={event=>selectTask(event,task)} onDoubleClick={event=>{event.stopPropagation();focusDate(new Date(task.start+"T00:00:00"));setSelectedTask(task.id);}} onMouseEnter={()=>setHoveredTask(task.id)} onMouseLeave={()=>setHoveredTask(null)}>{g.placeholder==="future"?"›":"‹"}</button>;
    ...
    return <TooltipProvider><Tooltip open={!!task.milestone&&hoveredTask===task.id&&!drag&&!resize&&!linkPreview}><TooltipTrigger asChild>
      <button data-task-motion={task.id} aria-label={t(task.title)} aria-pressed={selectedTask===task.id}
        ...
        onPointerCancel={()=>{clearInsertion();setDrag(null);}} onMouseEnter={()=>setHoveredTask(task.id)} onMouseLeave={()=>setHoveredTask(null)}
        ...
        {task.milestone ? <Diamond size={layoutTokens.arrowSize*2} className="-rotate-45"/> : <>
          ...
          {(["start","end"] as const).map(edge=><span key={edge} role="separator" aria-label={t("调整{0}的{1}日期", t(task.title), edge==="start" ? t("开始") : t("结束"))}
            onClick={event=>event.stopPropagation()} onPointerDown={event=>{event.stopPropagation();event.currentTarget.setPointerCapture(event.pointerId);setResize({id:task.id,edge,startX:event.clientX,previewDays:0,moved:false});}}
            onPointerMove={event=>moveResize(event,task)} onPointerUp={event=>finishResize(event,task)} onPointerCancel={()=>setResize(null)}
      ...
      </button></TooltipTrigger>{task.milestone&&<TooltipContent side="top" sideOffset={12} className="milestone-note"><span className="meta">{t("里程碑")}</span><div>{t(task.title)}</div><span className="meta">{task.start}</span></TooltipContent>}
  ...
  const deleteTodoItem=(item:InboxItem)=>{
    if(item.taskId){
      ...
      todoDeleteBackup.current={kind:"task",task,index,edges:edges.filter(edge=>edge.source.taskId===task.id||edge.target.taskId===task.id)};
  ...
  const undoTodoDelete=()=>{
    ...
    changeSchedule(current=>{
      ...
      const edgeIds=new Set(current.edges.map(edge=>edge.id));return {...current,tasks:next,edges:[...current.edges,...backup.edges.filter(edge=>!edgeIds.has(edge.id))]};
    });
    setNotice("已撤回删除。");
  };
  ...
  const confirmWorkTypeDelete=()=>{
    ...
    changeSchedule(current=>({...current,tasks:deleteWorkType(current.tasks,colors,name).tasks}));
  ...
  const exportJson=()=>{
    const blob=new Blob([JSON.stringify(createBackup(persistedState),null,2)],{type:"application/json"});
  ...
  return <main onClick={event => { if (!(event.target as Element).closest("button,input,select,textarea,[role]")) {setSelectedTask(null);setSelectedEdge(null);} }} className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
    ...
    <header ref={headerRef} className="toolbar border-b border-[var(--border)] bg-[var(--card)]">
      <div className="toolbar-brand">
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label={t("展开菜单")}><CalendarDays/></Button></DropdownMenuTrigger><DropdownMenuContent align="start">
      ...
      <div className="toolbar-actions flex-wrap">
        <div className="hidden items-center gap-1.5 rounded-md border border-[var(--border)] px-2 py-1 text-xs text-[var(--muted-foreground)] xl:flex" title={t("项目、事项和设置保存在当前浏览器；更换设备不会自动同步。")}><HardDrive className="size-3.5"/><span>{t("数据保存在此浏览器")}</span></div>
    ...
    <section className={`workspace-shell px-4 py-4 duration-200 motion-reduce:transition-none lg:px-6 ${settingsResizing?"transition-none":"transition-[margin,width]"}`} style={{"--workspace-inset":workspaceInset} as React.CSSProperties}>
      ...
      <div className="overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--card)]"><div ref={viewport} onDragOver={event=>{const target=inboxTarget(event);if(target){event.preventDefault();event.dataTransfer.dropEffect="link";trackInsertion(event.clientY);setInboxDrop({left:target.left,top:target.top,height:target.height});}else{setInboxDrop(null);clearInsertion();}}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node)){setInboxDrop(null);clearInsertion();}}} onDrop={dropInbox} onScroll={scrollViewport} className="overflow-auto timeline-viewport" style={{overflowAnchor:"none"}}><div className="grid min-w-max" style={{ gridTemplateColumns: `${PROJECT_COLUMN_WIDTH}px ${taskColumnWidth}px ${totalWidth}px` }}>
        ...
        <div className="sticky top-0 z-[95] flex h-24 items-end border-b border-r border-[var(--border)] table-task bg-[var(--table-task)] px-4 pb-3 text-sm font-semibold" style={{left:PROJECT_COLUMN_WIDTH}}>{t("事项")}{taskColumnDivider(0)}</div>
        ...
        <div className="daily-header" style={{gridColumn:3,gridRow:2}}><DailyAgenda dates={calendarDays.map(iso)} width={cellWidth/stepDays} tasks={tasks} inbox={inbox} colors={colors} onTask={openTask} onComplete={(id,done)=>changeSchedule(current=>({...current,inbox:current.inbox.map(item=>item.id===id?{...item,done}:item)}))} onReturn={id=>changeSchedule(current=>({...current,inbox:current.inbox.map(item=>item.id===id?{...item,date:undefined}:item)}))} dragging={!!inboxDragging} onDrop={scheduleInbox}/></div>
        <div className="sticky left-0 z-[90] table-project bg-[var(--table-project)]" style={{gridColumn:1,gridRow:3}}>{projectGroups.map(group=>{
          ...
          return <div key={group.project.id} className="project-card relative border-b border-r border-[var(--border)] table-project bg-[var(--table-project)] px-3 py-3" style={{height}}>
            <button onClick={()=>group.tasks.length&&toggleProject(group.project.id)} className="flex max-w-[112px] items-start gap-2 text-left"><ChevronDown size={15} className={collapsedProjects.has(group.project.id) ? "rotate-[-90deg]" : ""}/><span className="min-w-0"><span className="project-name block truncate">{t(group.project.name)}</span><span className="meta mt-1 block">{t("{0} 项 · {1} 行",group.tasks.length,group.rows.length)}</span></span></button>
            <DropdownMenu><DropdownMenuTrigger asChild><button className="project-actions absolute right-2 top-3 grid size-8 place-items-center rounded-lg" aria-label={t("{0}项目操作",t(group.project.name))}><Ellipsis size={17}/></button></DropdownMenuTrigger><DropdownMenuContent align="start"><DropdownMenuItem onSelect={()=>window.setTimeout(()=>openRenameProject(group.project),0)}><Pencil/>{t("重命名项目")}</DropdownMenuItem><DropdownMenuItem className="text-[var(--destructive)]" onSelect={()=>requestProjectDelete(group.project)}><Trash2/>{t("删除项目")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
          </div>;
        })}</div>
        <div className="sticky z-[80] table-task bg-[var(--table-task)]" style={{gridColumn:2,gridRow:3,height:rowsHeight,left:PROJECT_COLUMN_WIDTH}}>
          {visibleRows.map((row,index)=>{
            ...
            return <div key={row.project.id+key} data-row-key={"label-"+key} data-row-kind={row.kind} className="schedule-row absolute left-0 w-full border-b border-r border-[var(--border)] table-task bg-[var(--table-task)]" style={{opacity:row.kind==="task"&&row.tasks.every(task=>task.status==="已完成")&&completedMode==="fade"?appearance.completedOpacity/100:1,top:rowTops[index],height:rowHeight(row),transition:"height 180ms ease",backgroundColor:row.kind==="add"?"var(--table-add)":row.kind==="task"&&rowDrag&&rowId(row.task)===rowId(tasks.find(task=>task.id===rowDrag.taskId)??row.task)?"var(--accent)":undefined,color:row.kind==="add"?"var(--table-add-text)":undefined}}>
              {row.kind==="insert" ? <div className="insertion-slot flex h-full items-center overflow-hidden px-4 text-xs text-blue-700">{row.phase==="ready" ? t("松开放入新行") : ""}</div> : row.kind==="task" ? <div className="flex h-full items-center gap-2 px-3">
                <button aria-label={t("拖动整行：{0}", t(row.task.title))} onPointerDown={event=>startRowDrag(event,row.task,row.project.id)} onClick={event=>event.stopPropagation()} className="row-handle grid h-10 w-5 shrink-0 cursor-grab touch-none place-items-center text-slate-400"><GripVertical size={16}/></button>
                <button onClick={event=>selectTask(event,row.task)} onDoubleClick={()=>openTask(row.task)} className="min-w-0 flex-1 text-left"><span className={`task-row-name block truncate ${row.tasks.every(task=>task.status==="已完成")?"line-through":""}`}>{t(row.task.title)}</span><span className="row-details"><span className="category-dot" title={t(row.task.type)} style={{"--task-color":colors[row.task.type]} as React.CSSProperties}/><span className="status-tag" data-status={row.task.status}>{t(row.task.status)}</span>{row.tasks.length>1&&<span className="meta">{row.tasks.length} {" "}{t("个事项")}</span>}</span></button>
              </div> : row.kind==="summary" ? <button className="h-full w-full px-4 text-left text-sm" onClick={()=>toggleProject(row.project.id)}>{t("项目摘要 ·")}{" "}{row.tasks.length} {" "}{t("项")}</button> : <button className="add-task-link flex h-full w-full items-center gap-2 px-4 text-left text-xs" onClick={()=>addTaskToProject(row.project.id)}><Plus size={14}/>{row.kind==="add" ? t("此项目中的新事项") : t("添加第一个事项")}</button>}
            </div>;
          })}{taskColumnDivider()}
        </div>
        <div aria-hidden="true" className="pointer-events-none sticky left-0 z-10" style={{gridColumn:"1 / -1",gridRow:3,width:viewportWidth,height:0,alignSelf:"start"}}>
          {visibleRows.map((row,index)=>row.kind==="add"?<div key={row.project.id} style={{position:"absolute",left:frozenWidth,right:0,top:rowTops[index],height:layoutTokens.addRowHeight,background:"var(--table-add)",borderBottom:"1px solid var(--border)"}}/>:null)}
        </div>
<div ref={timeline} onDoubleClick={createAtPoint} className={`relative select-none ${panning ? "cursor-grabbing" : "cursor-grab"}`} style={{ gridColumn:3,gridRow:3,backgroundColor:"var(--table-canvas)",width: totalWidth, height:rowsHeight, touchAction:"pan-y",isolation:"isolate",overflow:"clip",zIndex:0,clipPath:`inset(0 0 0 ${scrollOffset}px)` }} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan} onLostPointerCapture={endPan} onClickCapture={event=>{if(suppressPanClick.current){event.preventDefault();event.stopPropagation();suppressPanClick.current=false;}}}>
          ...
          {todayOffset>=0&&todayOffset<=cellCount&&<div className="today-line" style={{left:todayOffset*cellWidth}}/>}
          <svg className="dependency-layer pointer-events-none absolute inset-0 z-[25] h-full w-full overflow-visible" aria-label={t("事项依赖关系")}>
            {[...connections].sort((a,b)=>Number(a.edge.id===selectedEdge)-Number(b.edge.id===selectedEdge)).map(connection=>{
              ...
              const stroke=related||isSelected?(colors[focused?.type??fallbackType]??fallbackColor):"var(--dependency-color)";
              ...
              return <g key={connection.key} style={{visibility:linkPreview?.edgeId===connection.edge.id?"hidden":undefined}} opacity={filtered ? .2 : 1}>
                ...
                <path d={connection.path} fill="none" stroke="transparent" strokeWidth={layoutTokens.edgeHit} pointerEvents="stroke" className="cursor-pointer" role="button" tabIndex={0} aria-label={t("选中任务连接线")} aria-pressed={isSelected}
      ...
      <div ref={settingsLayoutRef} className="settings-layout" style={{"--settings-nav-width":`${settingsNavWidth}px`} as React.CSSProperties}>
        ...
        <div className="settings-detail">
          ...
          {settingsCategory==="tasks"&&<>
            ...
            <fieldset className="settings-section"><legend>{t("工作类型")}</legend><div className="work-type-list">{Object.entries(colors).map(([type,color])=><div key={type} className="work-type-item"><span className="category-dot" style={{"--task-color":color} as React.CSSProperties}/><span>{t(type)}</span><button type="button" disabled={workTypes.length<=1} aria-label={t("删除工作类型：{0}",t(type))} onClick={()=>setPendingWorkTypeDelete(type)}><X size={14}/></button></div>)}</div><div className="custom-type-editor"><Input maxLength={24} value={customTypeName} onChange={event=>setCustomTypeName(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();addWorkType();}}} placeholder={t("工作类型名称")}/><input type="color" value={customTypeColor} aria-label={t("自定义工作类型颜色")} onChange={event=>setCustomTypeColor(event.target.value)}/><Button type="button" variant="outline" onClick={()=>addWorkType()}>{t("添加")}</Button></div></fieldset>
