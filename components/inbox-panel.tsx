...
import {extractInboxUrgency,inboxTimeTone,inboxUrgencyCount,isInboxItemOverdue,parseInboxInput,sortInboxItems,type InboxItem,type InboxKind,type InboxSort} from "@/lib/inbox";
...
export function InboxPanel({items,defaultKind,sort,collapsed,title,size,completedVisibility,onCollapse,onChange,onDelete,onUndoDelete,onSortChange,onDrag,onSizeChange}: {
  ...
  const editor=<div className="inbox-editor"><span aria-hidden="true">{draft?.kind==="numbered"?"1.":draft?.kind==="bullet"?"•":"○"}</span><input ref={input} autoFocus value={draft?.text??""} aria-label={t("条目内容")} placeholder={t("输入内容，Enter 保存")} onCompositionStart={()=>{composing.current=true;}} onCompositionEnd={event=>{composing.current=false;const text=event.currentTarget.value;setDraft(current=>current?{...current,...parseInboxInput(text,current.kind)}:null);}} onChange={event=>{const text=event.target.value;setDraft(current=>current?{...current,...(composing.current?{text}:parseInboxInput(text,current.kind))}:null);}} onBlur={()=>{if(cancelDraft.current){cancelDraft.current=false;return;}save();}} onKeyDown={event=>{if(event.nativeEvent.isComposing||composing.current)return;if(event.key==="Enter"){event.preventDefault();save();}if(event.key==="Escape"){event.stopPropagation();setDraft(null);}}}/><Tooltip><TooltipTrigger asChild><button type="button" className="inbox-editor-info" aria-label={t("优先级输入说明")} onPointerDown={event=>event.preventDefault()}><CircleHelp size={15}/></button></TooltipTrigger><TooltipContent>{t("在文本中加入 1–3 个 !，可快速标注优先级。")}</TooltipContent></Tooltip><button aria-label={t("取消")} onPointerDown={()=>{cancelDraft.current=true;}} onClick={()=>setDraft(null)}><X size={14}/></button></div>;
