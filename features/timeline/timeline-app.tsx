...
import {visibleMonthSegment} from "@/lib/presentation/calendar-header";
import { ChevronDown, ChevronLeft, ChevronRight, Circle, Cloud, Diamond, GripVertical, Link2, Plus, RefreshCw, Settings, Sun, Moon, Monitor, X, CircleHelp, Ellipsis, Globe, HardDrive, Download, Upload, ExternalLink, Copy, Pencil, Trash2, BookOpen, Check } from "lucide-react";
...
import { ManualDateField } from "@/components/shared/manual-date-field";
import { expandedInset, foldedSide, portOffset, visiblePortDays, taskBarWidth } from "@/lib/presentation/task-presentation";
import { ExpandingTaskLabel } from "@/features/timeline/expanding-task-label";
import {builtInColorPresets,nextCustomPresetName,restoreCustomColorPresets,type ColorPreset} from "@/lib/presentation/color-presets";

import {DailyAgenda} from "@/features/inbox/daily-agenda";
import {InboxPanel} from "@/features/inbox/inbox-panel";
import {restoreInbox,inboxKinds,type InboxItem,type InboxKind,type InboxSort} from "@/lib/domain/inbox";
import {projectTasksToTodo,reconcileTodoList} from "@/lib/domain/todo-projection";
import {Tooltip,TooltipProvider,TooltipTrigger,TooltipContent} from "@/components/ui/tooltip";
import {advanceScheduleDocument,backupFilename, createBackup, CURRENT_DATA_VERSION, loadScheduleDocument, migratePersistedState, parseBackup, type PersistedState,type ScheduleDocument} from "@/lib/persistence/persistence";
import {extractMemoLinks} from "@/lib/domain/memo-links";
import {clampSettingsNavWidth,SETTINGS_NAV_MAX,SETTINGS_NAV_MIN} from "@/lib/presentation/settings-layout";
import {clampTaskColumnWidth,collapsedSummaryBounds,COLLAPSED_PROJECT_HEIGHT,PROJECT_COLUMN_WIDTH,TASK_COLUMN_DEFAULT,TASK_COLUMN_MAX,TASK_COLUMN_MIN} from "@/lib/presentation/timeline-layout";
import {deleteWorkType,renameWorkType,renameWorkTypeColorMap} from "@/lib/domain/work-types";
...
import {activeTimelineId as loadActiveTimelineId,loadTimelineIndex,mergeTimelineEntries,readTimelineDocument,removeTimeline,saveTimelineIndex,setActiveTimelineId as rememberActiveTimelineId,upsertTimeline,writeTimelineDocument,type TimelineEntry} from "@/lib/persistence/timeline-library";
import {copyTaskSnapshot,parseTaskClipboard,taskFromClipboard,TASK_CLIPBOARD_STORAGE_KEY,type TaskClipboardPayload} from "@/lib/domain/task-clipboard";
import {routeAroundTasks,smoothRoute,type Point,type Rect} from "@/features/timeline/dependency-routing";
import {duration,emptyTask,initialEdges,initialProjects,initialTasks,iso,nextProjectColor,portDate,statusForRange,type PasteAnchor,type Project,type RenderedConnection,type Schedule,type VisibleRow} from "@/features/timeline/model";
...
type PersonalDefaults=Partial<{
  ...
  appearance:{theme:"light"|"dark"|"system";primaryColor:string;collapsedProjectColor:string;collapsedOpacity:number;colors:Record<string,string>;tablePalettes:TablePalettes};
}>;
...
const appearancePresetLabels:Record<keyof typeof appearancePresetValues,Array<[string,number]>>={
  ...
  collapsedOpacity:[["更淡",40],["标准",60],["清晰",80],["不透明",100]],
...
export function TimelineApp() {
  ...
  const [pendingTimelineDelete,setPendingTimelineDelete]=useState<TimelineEntry|null>(null);
  ...
  const [todayMarkerTime,setTodayMarkerTime]=useState(()=>new Date());
  ...
  const [settingsCategory,setSettingsCategory]=useState<"general"|"sync"|"timeline"|"tasks"|"links"|"inbox"|"appearance">("general");
  ...
  const [webDavService,setWebDavService]=useState<"nutstore"|"custom">(()=>{if(typeof window==="undefined")return "nutstore";const saved=loadWebDavConnection(window.localStorage);return saved&&!saved.url.startsWith("https://dav.jianguoyun.com/")?"custom":"nutstore";});
  const [webDavUrl,setWebDavUrl]=useState(()=>{if(typeof window==="undefined")return NUTSTORE_WEBDAV_URL;return loadWebDavConnection(window.localStorage)?.url??NUTSTORE_WEBDAV_URL;});
  const [webDavUsername,setWebDavUsername]=useState(()=>{if(typeof window==="undefined")return "";return loadWebDavConnection(window.localStorage)?.username??"";});
  const [webDavPassword,setWebDavPassword]=useState(()=>{if(typeof window==="undefined")return "";return loadWebDavConnection(window.localStorage)?.password??"";});
  ...
  useEffect(()=>{
    ...
    const storedValue=localStorage.getItem(SYNC_PROVIDER_KEY);
    const stored:SyncProvider=storedValue==="sites"||storedValue==="webdav"?storedValue:"local";
    void loadSyncAccount().then(account=>{
      if(!active)return;
      setSyncAccount(account);setSyncAccountChecked(true);
      if(account&&(requested||stored!=="local")){
        const provider=stored==="local"?"sites":stored;
        localStorage.setItem(SYNC_PROVIDER_KEY,provider);
        setSyncProvider(provider);setWebDavEnabled(provider==="webdav"&&!!loadWebDavConnection(localStorage));setSyncStatus("idle");
      }else if(stored!=="local"&&!account){
        localStorage.removeItem(SYNC_PROVIDER_KEY);
        setSyncProvider("local");setWebDavEnabled(false);setSyncStatus("local");
      }
      if(requested){const url=new URL(window.location.href);url.searchParams.delete("sync");window.history.replaceState({},"",`${url.pathname}${url.search}${url.hash}`);}
  ...
  useEffect(()=>{
    ...
    root.style.setProperty("--collapsed-summary-opacity",String(appearance.collapsedOpacity/100));
    root.style.setProperty("--collapsed-summary-hover-opacity",String(Math.min(100,appearance.collapsedOpacity+20)/100));
  ...
  useEffect(()=>{if(notice)return;queueMicrotask(()=>setNoticeHovered(false));},[notice]);
  ...
  useEffect(() => {
    ...
    void (async()=>{try {
      const library=loadTimelineIndex(localStorage);
      const requestedId=loadActiveTimelineId(localStorage);
      ...
      const legacyStorage=createBrowserScheduleStorage<unknown>(window.localStorage);
      const unified = (await legacyStorage.load()).data;
      let document:ScheduleDocument|null=null;
      if (unified) {
        const loaded=loadScheduleDocument(unified,deviceIdRef.current);
        document={...loaded.document,title:loaded.document.title||"我的 TimeLine"};
  ...
  const activeCloudContext=()=>{
    ...
    if(syncProvider==="webdav"){
      const connection=loadWebDavConnection(localStorage);if(!connection)return null;
      return {store:new WebDavScheduleStore(connection),scope:webDavCheckpointScope(connection)};
    }
    return null;
  };
  const applyCloudDocument=async(document:ScheduleDocument,serverRevision:CloudRevision)=>{
  ...
  const performCloudSync=async()=>{
    ...
    try{
      ...
      if(!checkpoint)throw new Error("sync_checkpoint_missing");
      checkpoints.save(checkpoint);
      if(result.status==="downloaded")await applyCloudDocument(result.document,checkpoint.serverRevision);
  ...
  const scheduleCloudSync=(delay=1200)=>{
    ...
    cloudSyncTimer.current=setTimeout(()=>void performCloudSync(),delay);
  };
  const enableCloudSync=()=>{
    if(!syncAccount){window.location.assign("/sync");return;}
    localStorage.setItem(SYNC_PROVIDER_KEY,"sites");setWebDavEnabled(false);setSyncProvider("sites");setLastSyncedAt(null);setSyncStatus("idle");
  };
  const disableCloudSync=()=>{
    if(cloudSyncTimer.current)clearTimeout(cloudSyncTimer.current);
    localStorage.removeItem(SYNC_PROVIDER_KEY);setWebDavEnabled(false);setSyncProvider("local");setLastSyncedAt(null);setSyncStatus("local");setPendingCloudChoice(null);
  };
  const selectWebDav=()=>{
    if(cloudSyncTimer.current)clearTimeout(cloudSyncTimer.current);
    localStorage.removeItem(SYNC_PROVIDER_KEY);setWebDavEnabled(false);setSyncProvider("webdav");setLastSyncedAt(null);setSyncStatus("local");setPendingCloudChoice(null);
  };
  const connectWebDav=async()=>{
    const connection:WebDavConnection={url:webDavService==="nutstore"?NUTSTORE_WEBDAV_URL:webDavUrl,username:webDavUsername,password:webDavPassword};
    const error=validateWebDavConnection(connection);
    if(error){setNotice(t(error==="webdav_credentials_required"?"请填写 WebDAV 账号和应用密码。":error==="webdav_file_url_required"?"WebDAV 地址需要指向一个 JSON 文件。":"请输入有效的 HTTPS WebDAV 文件地址。"));return;}
    if(!syncAccount){saveWebDavConnection(localStorage,connection);localStorage.setItem(SYNC_PROVIDER_KEY,"webdav");window.location.assign("/sync");return;}
    setWebDavTesting(true);setSyncStatus("syncing");
    try{
      await new WebDavScheduleStore(connection).load(scheduleDocumentRef.current?.documentId??"");
      saveWebDavConnection(localStorage,connection);
      localStorage.setItem(SYNC_PROVIDER_KEY,"webdav");setSyncProvider("webdav");setWebDavEnabled(true);setSyncStatus("idle");setNotice("WebDAV 已连接，正在进行首次同步。");
    }catch(error){
      setSyncStatus("error");
      setNotice(error instanceof SyncRequestError&&error.code==="webdav_auth_failed"?"WebDAV 账号或应用密码不正确。":error instanceof SyncRequestError&&error.code==="webdav_invalid_document"?"目标文件已存在，但不是有效的 Squitle 数据。":"无法连接 WebDAV，请检查地址和网络。");
    }finally{setWebDavTesting(false);}
  };
  const chooseLocalForCloud=async()=>{
    const choice=pendingCloudChoice;if(!choice)return;
    try{
      preserveConflictBackup(choice.remote.document,"cloud");setSyncStatus("syncing");
      const context=activeCloudContext();if(!context)throw new Error("sync_provider_unavailable");
      const snapshot=await context.store.save(choice.local,choice.remote.serverRevision);
      new BrowserSyncCheckpointStore(localStorage,context.scope).save(checkpointFrom(snapshot));
      setPendingCloudChoice(null);setLastSyncedAt(new Date().toISOString());setSyncStatus("synced");setNotice("已使用此设备的数据，云端原版本保存在浏览器恢复副本中。");
    }catch{setSyncStatus("error");setNotice("云端数据已变化，请重新同步后再选择。");setPendingCloudChoice(null);}
  };
  const chooseCloudForLocal=async()=>{
    const choice=pendingCloudChoice;if(!choice)return;
    try{
      preserveConflictBackup(choice.local,"local");setSyncStatus("syncing");
      await applyCloudDocument(choice.remote.document,choice.remote.serverRevision);
  ...
  const rowTops=useMemo(()=>visibleRows.reduce<{tops:number[];next:number}>((result,row)=>({tops:[...result.tops,result.next],next:result.next+rowHeight(row)}),{tops:[],next:0}).tops,[visibleRows,timelineRowHeight]);
  ...
  useEffect(()=>{
    ...
    const refresh=()=>{
      const now=new Date();
      setTodayMarkerTime(now);
      if(timer)clearTimeout(timer);
      timer=setTimeout(refresh,millisecondsUntilNextTodayMarker(now)+25);
    };
    const refreshWhenVisible=()=>{if(document.visibilityState==="visible")refresh();};
    refresh();
    document.addEventListener("visibilitychange",refreshWhenVisible);
    return()=>{if(timer)clearTimeout(timer);document.removeEventListener("visibilitychange",refreshWhenVisible);};
  },[]);
  const todayOffset = (differenceInCalendarDays(startOfDay(todayMarkerTime), anchor) + todayMarkerFraction(todayMarkerTime)) / stepDays;
  ...
  const selectTask = (event: React.MouseEvent, task: Task) => {
    ...
    setSelectedTask(task.id); setSelectedEdge(null);setPasteAnchor(null);
  };
  ...
  const rememberTaskClipboard=useCallback((payload:TaskClipboardPayload|null)=>{
    taskClipboardRef.current=payload;
    try{if(payload)sessionStorage.setItem(TASK_CLIPBOARD_STORAGE_KEY,JSON.stringify(payload));else sessionStorage.removeItem(TASK_CLIPBOARD_STORAGE_KEY);}catch{}
  },[]);
  const readTaskClipboard=useCallback(()=>{
    if(taskClipboardRef.current)return taskClipboardRef.current;
    let payload:TaskClipboardPayload|null=null;
    try{payload=parseTaskClipboard(sessionStorage.getItem(TASK_CLIPBOARD_STORAGE_KEY));}catch{}
    taskClipboardRef.current=payload;
    return payload;
  },[]);
  const copySelectedTask=useCallback((kind:"copy"|"cut")=>{
    const task=tasks.find(item=>item.id===selectedTask);if(!task)return;
    rememberTaskClipboard({version:1,kind,sourceDocumentId:activeTimelineId,task:copyTaskSnapshot(task),edges:kind==="cut"?edges.filter(edge=>edge.source.taskId===task.id||edge.target.taskId===task.id).map(edge=>({...edge,source:{...edge.source},target:{...edge.target},outputIds:edge.outputIds?[...edge.outputIds]:undefined})):undefined});
    if(kind==="cut"){
      deleteTask(task.id);
      setNotice(t("已剪切事项“{0}”，可切换 TimeLine 后粘贴。",t(task.title)));
    }else setNotice(t("已复制事项“{0}”。",t(task.title)));
  },[activeTimelineId,deleteTask,edges,rememberTaskClipboard,selectedTask,t,tasks]);
  const pasteTaskClipboard=useCallback(()=>{
    const payload=readTaskClipboard();
    if(!payload){setNotice("没有可粘贴的事项。");return;}
    const activeAnchor=pasteAnchor?.documentId===activeTimelineId&&projects.some(project=>project.id===pasteAnchor.projectId)?pasteAnchor:null;
    const selected=tasks.find(task=>task.id===selectedTask);
    const projectId=activeAnchor?.projectId??selected?.projectIds.find(id=>projects.some(project=>project.id===id))??payload.task.projectIds.find(id=>projects.some(project=>project.id===id))??projects[0]?.id;
    if(!projectId){setNotice("请先创建项目，再粘贴事项。");return;}
    const pasted=taskFromClipboard({payload,tasks,selectedTask:selected,anchor:activeAnchor?{start:activeAnchor.date,rowId:activeAnchor.rowId,order:activeAnchor.order}:undefined,projectId,currentDocumentId:activeTimelineId,allowOverlap:allowTaskOverlap,createId:createLocalId});
    changeSchedule(current=>{
      const nextTasks=[...current.tasks,pasted];
      const taskIds=new Set(nextTasks.map(task=>task.id));
      const restoredEdges=payload.kind==="cut"&&payload.sourceDocumentId===activeTimelineId&&pasted.id===payload.task.id?(payload.edges??[]).filter(edge=>taskIds.has(edge.source.taskId)&&taskIds.has(edge.target.taskId)&&!current.edges.some(currentEdge=>currentEdge.id===edge.id)):[];
      return {...current,tasks:nextTasks,edges:[...current.edges,...restoredEdges]};
    });
    if(payload.kind==="cut")rememberTaskClipboard(null);
    setSelectedTask(pasted.id);setSelectedEdge(null);setHoveredTask(null);setPendingPort(null);setPasteAnchor(null);
    setNotice(activeAnchor&&pasted.start!==activeAnchor.date?t("点击位置有重叠，已从 {0} 开始粘贴“{1}”。",pasted.start,t(pasted.title)):t("已粘贴事项“{0}”。",t(pasted.title)));
  },[activeTimelineId,allowTaskOverlap,changeSchedule,pasteAnchor,projects,readTaskClipboard,rememberTaskClipboard,selectedTask,t,tasks]);
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      } else if ((event.ctrlKey || event.metaKey) && !event.altKey && !event.shiftKey && !event.repeat && (key === "c" || key === "x") && selectedTask) {
        event.preventDefault();copySelectedTask(key==="x"?"cut":"copy");
      } else if ((event.ctrlKey || event.metaKey) && !event.altKey && !event.shiftKey && !event.repeat && key === "v") {
        event.preventDefault();pasteTaskClipboard();
      ...
      } else if (key === "escape") { setSelectedTask(null); setSelectedEdge(null); setPasteAnchor(null); setPendingPort(null); }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [selectedTask, selectedEdge, changeSchedule, deleteTask, copySelectedTask, pasteTaskClipboard, editing, newProjectOpen, settingsOpen, ready, drag, resize, rowDrag]);
  ...
  const choosePasteAnchor=(event:React.MouseEvent<HTMLDivElement>)=>{
    if((event.target as Element).closest('button,input,select,textarea,[role="button"],[role="separator"]')||suppressPanClick.current||linkGesture.current)return;
    const rect=event.currentTarget.getBoundingClientRect(),index=rowAtY(event.clientY-rect.top),row=visibleRows[index];
    if(!row||row.kind==="insert"||row.kind==="summary")return;
    const day=Math.floor((event.clientX-rect.left)/cellWidth*stepDays);
    setPasteAnchor({documentId:activeTimelineId,projectId:row.project.id,rowId:row.kind==="task"?rowId(row.task):undefined,order:row.kind==="task"?row.task.order[row.project.id]:undefined,rowKind:row.kind,date:iso(addDays(anchor,day))});
    setSelectedTask(null);setSelectedEdge(null);setHoveredTask(null);setHoveredEdge(null);
  };
  ...
  const switchTimeline=async(documentId:string)=>{
    if(documentId===activeTimelineId)return;
    try{
      await saveQueueRef.current.catch(()=>undefined);
      let document=readTimelineDocument(localStorage,documentId);
      let cloudRevision:CloudRevision|null=null;
      if(!document&&syncAccount){const snapshot=await new SitesScheduleStore().load(documentId);if(snapshot){document=snapshot.document;cloudRevision=snapshot.serverRevision;writeTimelineDocument(localStorage,document);}}
      if(!document){setNotice("这个 TimeLine 暂时无法打开。");return;}
      rememberActiveTimelineId(localStorage,documentId);scheduleStorageRef.current=createBrowserScheduleStorage<unknown>(localStorage,timelineDocumentKey(documentId));scheduleDocumentRef.current=document;
      const selected=loadTimelineIndex(localStorage).find(item=>item.documentId===documentId);const updated=selected?upsertTimeline(localStorage,{...selected,title:document.title||selected.title,updatedAt:document.updatedAt,cloudOnly:false,cloudBacked:selected.cloudBacked||cloudRevision!==null}):loadTimelineIndex(localStorage);
      setActiveTimelineId(documentId);setTimelines(updated);applyPersistedState(document.data);
      if(cloudRevision!==null)new BrowserSyncCheckpointStore(localStorage,"sites").save(checkpointFrom({documentId,serverRevision:cloudRevision,updatedAt:document.updatedAt,document}));
      setSelectedTask(null);setSelectedEdge(null);setPasteAnchor(null);setEditing(null);setNotice(`已打开“${document.title||"我的 TimeLine"}”。`);
    }catch{setNotice("这个 TimeLine 暂时无法打开。");}
  };
  ...
  const keepCurrentTimeline=async()=>{
    if(!syncAccount){window.location.assign("/sync");return;}
    try{
      await saveQueueRef.current.catch(()=>undefined);const document=scheduleDocumentRef.current;if(!document)return;
      setSyncStatus("syncing");const snapshot=await new SitesScheduleStore().save(document,null);new BrowserSyncCheckpointStore(localStorage,"sites").save(checkpointFrom(snapshot));
      const updated=upsertTimeline(localStorage,{documentId:document.documentId,title:document.title||"示例 TimeLine",updatedAt:snapshot.updatedAt,temporary:false,cloudBacked:true});setTimelines(updated);localStorage.setItem(SYNC_PROVIDER_KEY,"sites");setWebDavEnabled(false);setSyncProvider("sites");setLastSyncedAt(snapshot.updatedAt);setSyncStatus("synced");setNotice("这个 TimeLine 已保存到你的账号。");
    }catch(error){setSyncStatus("error");setNotice(error instanceof SyncRequestError&&error.code==="revision_conflict"?"这个 TimeLine 已存在，请刷新后重试。":"暂时无法保存到账号，本地内容仍然保留。");}
  };
  const commitTimelineRename=async(entry:TimelineEntry)=>{
    const title=(timelineDrafts[entry.documentId]??entry.title).trim().slice(0,80);
    if(!title){setNotice("TimeLine 名称不能为空。");return;}
    if(timelines.some(item=>item.documentId!==entry.documentId&&item.title.toLocaleLowerCase()===title.toLocaleLowerCase())){setNotice("这个 TimeLine 名称已经存在。");return;}
    try{
      await saveQueueRef.current.catch(()=>undefined);
      const document=readTimelineDocument(localStorage,entry.documentId);
      if(document){const renamed={...document,title};writeTimelineDocument(localStorage,renamed);if(entry.documentId===activeTimelineId)scheduleDocumentRef.current=renamed;}
      let updatedAt=new Date().toISOString();
      if(syncAccount&&entry.cloudBacked){const result=await renameCloudTimeline(entry.documentId,title);updatedAt=result.timeline.updatedAt;}
      const updated=upsertTimeline(localStorage,{...entry,title,updatedAt});setTimelines(updated);setTimelineDrafts(current=>{const next={...current};delete next[entry.documentId];return next;});
      if(entry.documentId===activeTimelineId&&syncProvider==="sites")scheduleCloudSync(100);
      setNotice("TimeLine 已重命名。");
    }catch{setNotice("暂时无法重命名这个 TimeLine。");}
  };
  const confirmTimelineDelete=async()=>{
    const entry=pendingTimelineDelete;if(!entry)return;
    if(timelines.length<=1){setPendingTimelineDelete(null);setNotice("至少需要保留一个 TimeLine。");return;}
    try{
      if(cloudSyncTimer.current)clearTimeout(cloudSyncTimer.current);
      await saveQueueRef.current.catch(()=>undefined);
      if(syncAccount&&entry.cloudBacked)await deleteCloudTimeline(entry.documentId);
      const remaining=removeTimeline(localStorage,entry.documentId);localStorage.removeItem(timelineDocumentKey(entry.documentId));setTimelines(remaining);setPendingTimelineDelete(null);
      setTimelineDrafts(current=>{const next={...current};delete next[entry.documentId];return next;});
      if(entry.documentId===activeTimelineId&&remaining[0])await switchTimeline(remaining[0].documentId);
      setNotice(`已删除“${entry.title}”。`);
    }catch(error){setPendingTimelineDelete(null);setNotice(error instanceof SyncRequestError&&error.code==="last_timeline"?"云端至少需要保留一个 TimeLine。":"暂时无法删除这个 TimeLine。");}
  };
  ...
  const currentTimeline=timelines.find(item=>item.documentId===activeTimelineId);
  const inboxTitle="TodoList";
  ...
  const resetSettings=(category= settingsCategory)=>{
    if(category==="general"){const d=personalDefaults.general;setEditorMode(d?.editorMode??"panel");}
    ...
    if(category==="appearance"){const d=personalDefaults.appearance;setTheme(d?.theme??"system");setPrimaryColor(d?.primaryColor??"#4C7EF3");setCollapsedProjectColor(d?.collapsedProjectColor??categoryDefaults.整理);setAppearance(current=>({...current,collapsedOpacity:snapAppearanceValue("collapsedOpacity",d?.collapsedOpacity??60)}));setColors(d?.colors?restoreCategories(d.colors):categoryDefaults);setTablePalettes(d?.tablePalettes?migrateTablePalettes(d.tablePalettes,2):defaultTablePalettes);}
  };
  const saveCategoryDefault=()=>{
    const value=settingsCategory==="general"?{editorMode}:settingsCategory==="timeline"?{weekStart,dayWidth,timelineRowHeight,insertionDelay,weekends:appearance.weekends,weekBoundaries:appearance.weekBoundaries}:settingsCategory==="tasks"?{defaultDays,defaultTaskType,allowTaskOverlap,hoverSpeed,taskHoverHints,completedMode,barHeight:appearance.barHeight,barRadius:appearance.barRadius,barFill:appearance.barFill,completedOpacity:appearance.completedOpacity}:settingsCategory==="links"?{handleGap,handleWidth,autoConnectionSides,lineOpacity:appearance.lineOpacity,edgeLevel,arrowLevel}:settingsCategory==="inbox"?{inboxKind,inboxCompleted}:{theme,primaryColor,collapsedProjectColor,collapsedOpacity:appearance.collapsedOpacity,colors:{...colors},tablePalettes:structuredClone(tablePalettes)};
  ...
  const syncStatusLabel=syncProvider==="local"?t("数据保存在此浏览器"):t(syncStatus==="syncing"?"正在同步":syncStatus==="synced"?"云端已同步":syncStatus==="conflict"?"同步需要处理":"云同步未完成");
  ...
  return <main onClick={event => { if (!(event.target as Element).closest("button,input,select,textarea,[role]")) {setSelectedTask(null);setSelectedEdge(null);if(!(event.target as Element).closest("[data-timeline-canvas]"))setPasteAnchor(null);} }} className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
    ...
    <header ref={headerRef} className="toolbar border-b border-[var(--border)] bg-[var(--card)]">
      <div className="toolbar-brand">
        ...
        <span className="toolbar-wordmark" aria-label="SquiTLe">SquiTLe</span>
        ...
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="max-w-52 gap-1 px-2 font-semibold"><span className="truncate">{currentTimeline?.title||"TimeLine"}</span><ChevronDown className="size-4 shrink-0"/></Button></DropdownMenuTrigger><DropdownMenuContent align="start" className="min-w-56">{timelines.map(item=><DropdownMenuItem key={item.documentId} disabled={item.documentId===activeTimelineId} onSelect={()=>void switchTimeline(item.documentId)}><span className="min-w-0 flex-1 truncate">{item.title}</span>{item.temporary?<span className="meta">{t("临时")}</span>:item.cloudOnly?<Cloud className="size-3.5"/>:item.documentId===activeTimelineId?<Check className="size-3.5"/>:null}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>
      </div>
      <div className="toolbar-actions flex-wrap">
        {currentTimeline?.temporary&&<Button size="sm" variant="outline" onClick={()=>void keepCurrentTimeline()}>{syncAccount?t("保留此 TimeLine"):t("登录后保留")}</Button>}
        <button className="hidden items-center gap-1.5 rounded-md border border-[var(--border)] px-2 py-1 text-xs text-[var(--muted-foreground)] xl:flex" title={syncProvider==="local"?t("项目、事项和设置保存在当前浏览器；更换设备不会自动同步。"):syncAccount?.email} onClick={()=>{setSettingsCategory("sync");setEditing(null);setNewProjectOpen(false);setSettingsOpen(true);}}>{syncProvider==="sites"?<Cloud className="size-3.5"/>:<HardDrive className="size-3.5"/>}<span>{syncStatusLabel}</span></button>
        ...
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label={t("外观与更多设置")}><Ellipsis/></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onSelect={()=>void openExample()}><BookOpen/>{t("打开示例")}</DropdownMenuItem>
        </DropdownMenuContent></DropdownMenu>
      </div>
    </header>
    <section className={`workspace-shell px-4 py-4 duration-200 motion-reduce:transition-none lg:px-6 ${settingsResizing?"transition-none":"transition-[margin,width]"}`} style={{"--workspace-inset":workspaceInset} as React.CSSProperties}>
      ...
      <div className="overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--card)]"><div ref={viewport} onDragOver={event=>{const target=inboxTarget(event);if(target){event.preventDefault();event.dataTransfer.dropEffect="link";trackInsertion(event.clientY);setInboxDrop({left:target.left,top:target.top,height:target.height});}else{setInboxDrop(null);clearInsertion();}}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node)){setInboxDrop(null);clearInsertion();}}} onDrop={dropInbox} onScroll={scrollViewport} className="overflow-auto timeline-viewport" style={{overflowAnchor:"none"}}><div className="grid min-w-max" style={{ gridTemplateColumns: `${PROJECT_COLUMN_WIDTH}px ${taskColumnWidth}px ${totalWidth}px` }}>
        ...
        <div className="sticky top-0 z-[70] h-[88px] overflow-clip border-b border-[var(--border)] table-date bg-[var(--table-date)] date-header">
          ...
          <div className="flex h-14">{dates.map(date=><div key={date.toISOString()} className="relative shrink-0 px-2 py-1.5" style={{width:cellWidth}}>
...
<div ref={timeline} data-timeline-canvas onDoubleClick={createAtPoint} onClick={choosePasteAnchor} className={`relative select-none ${panning ? "cursor-grabbing" : "cursor-grab"}`} style={{ gridColumn:3,gridRow:3,backgroundColor:"var(--table-canvas)",width: totalWidth, height:rowsHeight, touchAction:"pan-y",isolation:"isolate",overflow:"clip",zIndex:0,clipPath:`inset(0 0 0 ${scrollOffset}px)` }} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan} onLostPointerCapture={endPan} onClickCapture={event=>{if(suppressPanClick.current){event.preventDefault();event.stopPropagation();suppressPanClick.current=false;}}}>
          ...
          {pasteAnchor?.documentId===activeTimelineId&&(()=>{const day=differenceInCalendarDays(new Date(pasteAnchor.date+"T00:00:00"),anchor),index=visibleRows.findIndex(row=>row.project.id===pasteAnchor.projectId&&(pasteAnchor.rowKind==="task"?row.kind==="task"&&rowId(row.task)===pasteAnchor.rowId:row.kind===pasteAnchor.rowKind));if(index<0||day<0||day>=cellCount*stepDays)return null;return <div aria-hidden="true" className="pointer-events-none absolute z-[18] rounded-sm border border-dashed border-[var(--primary)]" style={{left:day*cellWidth/stepDays,top:rowTops[index],width:cellWidth/stepDays,height:rowHeight(visibleRows[index]),backgroundColor:"color-mix(in srgb, var(--primary) 12%, transparent)"}}/>;})()}
    ...
    <Dialog modal={!panelMode} open={settingsOpen} onOpenChange={open=>{if(!open)captureSettingsWidth();setSettingsOpen(open);}}><DialogContent ref={settingsPanelRef} onKeyDown={event=>saveOnEnter(event,finishSettings)} {...settingsPresentation} data-panel={panelMode ? "true" : undefined} className={`max-h-[90vh] bg-[var(--card)] sm:max-w-[900px] ${panelMode?"settings-resizable":""}`}><DialogHeader><DialogTitle>{t("设置")}</DialogTitle><DialogDescription>{t("按类别调整 TimeLine、事项和 TodoList。")}</DialogDescription></DialogHeader>
      <div ref={settingsLayoutRef} className="settings-layout" style={{"--settings-nav-width":`${settingsNavWidth}px`} as React.CSSProperties}>
        <nav className="settings-nav" aria-label={t("设置分类")}>{([['general',t('通用')],['sync',t('云同步')],['timeline',timelineTitle],['tasks',t('事项')],['links',t('连线与操作')],['inbox',inboxTitle],['appearance',t('外观与颜色')]] as const).map(([value,label])=><button key={value} aria-current={settingsCategory===value?"page":undefined} onClick={()=>setSettingsCategory(value)}>{label}</button>)}</nav>
        ...
        <div className="settings-detail">
          {settingsCategory==="general"&&<>
            ...
            <div className="setting-row"><div><strong>{t("数据存储")}</strong><p>{t("项目、事项和设置保存在当前浏览器。请定期导出 JSON 备份；清除浏览器数据或更换设备不会自动同步。")}</p></div><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={chooseJsonImport}><Upload className="size-4"/>{t("导入 JSON")}</Button><Button variant="outline" onClick={exportJson}><Download className="size-4"/>{t("导出 JSON")}</Button></div></div>
          </>}
          {settingsCategory==="sync"&&<>
            ...
            {syncProvider==="webdav"&&<fieldset className="settings-section"><legend>{t("WebDAV 连接")}</legend>
              ...
              <div className="setting-row"><div><strong>{t("账号")}</strong><p>{t(webDavService==="nutstore"?"填写坚果云登录邮箱。":"填写 WebDAV 用户名。")}</p></div><Input data-enter-action="local" className="max-w-xs" value={webDavUsername} autoCapitalize="none" spellCheck={false} onChange={event=>setWebDavUsername(event.target.value)}/></div>
              <div className="setting-row"><div><strong>{t("应用密码")}</strong><p>{t(webDavService==="nutstore"?"请使用坚果云第三方应用密码，不要填写账户登录密码。":"请使用专门为 Squitle 创建的应用密码。")}</p></div><Input data-enter-action="local" className="max-w-xs" type="password" value={webDavPassword} autoComplete="off" onChange={event=>setWebDavPassword(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&!event.nativeEvent.isComposing){event.preventDefault();void connectWebDav();}}}/></div>
              <div className="setting-row"><p className="meta">{t("连接信息只保存在当前浏览器，不写入 Squitle 云数据库。同步请求由 Squitle 后端临时转发。")}</p><Button variant="outline" disabled={webDavTesting} onClick={()=>void connectWebDav()}>{webDavTesting?<RefreshCw className="size-4 animate-spin"/>:<Link2 className="size-4"/>}{t(syncAccount?webDavEnabled?"重新连接":"连接并启用":"登录并连接")}</Button></div>
            </fieldset>}
            {!syncAccountChecked?<p className="meta">{t("正在检查登录状态…")}</p>:!syncAccount?(syncProvider==="webdav"?<p className="meta">{t("填写连接信息后，点击“登录并连接”。登录只用于保护同步接口，WebDAV 数据由你的 WebDAV 账号控制。")}</p>:<div className="setting-row"><div><strong>{t("登录后开启")}</strong><p>{t("不启用同步时无需账号；开启后，每个账号只读取自己的数据。")}</p></div><Button asChild><a href="/sync">{t("使用 ChatGPT 登录")}</a></Button></div>):<>
              <div className="setting-row"><div><strong>{t("同步账号")}</strong><p>{syncAccount.email}</p></div><Button variant="outline" asChild><a href="/sync/signout">{t("退出登录")}</a></Button></div>
              <div className="setting-row"><div><strong>{t("同步状态")}</strong><p>{lastSyncedAt?t("上次同步：{0}",new Date(lastSyncedAt).toLocaleString(language==="zh"?"zh-CN":"en-US")):t("尚未完成首次同步")}</p></div><Button variant="outline" disabled={syncProvider==="local"||(syncProvider==="webdav"&&!webDavEnabled)||syncStatus==="syncing"} onClick={()=>void performCloudSync()}><RefreshCw className={`size-4 ${syncStatus==="syncing"?"animate-spin":""}`}/>{t(syncStatus==="syncing"?"正在同步":"立即同步")}</Button></div>
            </>}
            <fieldset className="settings-section"><legend>{t("同步安全")}</legend><p className="meta">{t("首次连接或两台设备同时修改时，Squitle 会让你选择保留哪个版本，并在当前浏览器保存被替换版本的恢复副本。")}</p></fieldset>
          </>}
          {settingsCategory==="timeline"&&<>
            <h3>{timelineTitle}</h3>
            <fieldset className="settings-section"><legend>{t("TimeLine 管理")}</legend><div className="timeline-manager-list">{timelines.map(entry=><div key={entry.documentId} className="timeline-manager-row" data-active={entry.documentId===activeTimelineId||undefined}><Button type="button" size="sm" variant={entry.documentId===activeTimelineId?"secondary":"outline"} onClick={()=>void switchTimeline(entry.documentId)} disabled={entry.documentId===activeTimelineId}>{t(entry.documentId===activeTimelineId?"当前":"打开")}</Button><Input data-enter-action="local" maxLength={80} value={timelineDrafts[entry.documentId]??entry.title} onChange={event=>setTimelineDrafts(current=>({...current,[entry.documentId]:event.target.value}))} onKeyDown={event=>{if(event.key==="Enter"&&!event.nativeEvent.isComposing){event.preventDefault();void commitTimelineRename(entry);}}} aria-label={t("TimeLine 名称")}/><Button type="button" size="icon" variant="ghost" aria-label={t("保存名称")} onClick={()=>void commitTimelineRename(entry)}><Check className="size-4"/></Button><Button type="button" size="icon" variant="ghost" className="text-[var(--destructive)]" aria-label={t("删除 TimeLine：{0}",entry.title)} disabled={timelines.length<=1} onClick={()=>setPendingTimelineDelete(entry)}><Trash2 className="size-4"/></Button></div>)}</div><p className="meta">{t("选择要编辑的 TimeLine；名称会随云同步保存。至少保留一个 TimeLine。")}</p></fieldset>
          ...
          {settingsCategory==="tasks"&&<>
            ...
            {Object.entries(appearanceFields).filter(([key])=>["barHeight","barRadius","barFill","completedOpacity"].includes(key)).map(([key,field])=>{const name=key as keyof typeof appearancePresetValues;return <div key={key} className="setting-row"><div><strong>{t(field.label)}</strong></div><PresetTabs value={appearance[name]} onChange={value=>setAppearance(current=>({...current,[name]:Number(value)}))} options={appearancePresetLabels[name].map(([label,value])=>[t(label),value])}/></div>;})}
          </>}
          ...
          {settingsCategory==="appearance"&&<>
            ...
            <div className="setting-row"><div><strong>{t("折叠事项不透明度")}</strong><p>{t("控制项目折叠后右侧摘要事项条的透明程度。")}</p></div><PresetTabs value={appearance.collapsedOpacity} onChange={value=>setAppearance(current=>({...current,collapsedOpacity:Number(value)}))} options={appearancePresetLabels.collapsedOpacity.map(([label,value])=>[t(label),value])}/></div>
