...
test('built-in presets contain the requested day and night themes',()=>{
  ...
  for(const item of builtInColorPresets){assert.match(item.primary,/^#[0-9a-f]{6}$/i);assert.equal(Object.keys(item.categories).length,6);assert.equal(Object.keys(item.table).length,7);}
});
test('custom preset restoration rejects incomplete or invalid palettes',()=>{
  const valid={id:'mine',name:'  My colors  ',mode:'dark',categories:{论文:'#112233',实验:'#223344',工程:'#334455',其他:'#445566'},table:{project:'#111111',task:'#121212',month:'#131313',date:'#141414',canvas:'#151515',add:'#161616',grid:'#171717'}};
