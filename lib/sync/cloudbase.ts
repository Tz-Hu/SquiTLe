...
export class CloudBaseScheduleStore implements CloudScheduleStore{
  ...
  private async call(data:Record<string,unknown>){
    ...
    const response=await this.client.callFunction({name:this.functionName,data});
    const result=response.result as SyncFunctionResult;
    if(!result?.ok)throw new Error(result?.error||"cloudbase_sync_failed");
