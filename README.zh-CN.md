...
SquiTLe 是一个本地优先的个人时间轴排期工具。打开网页就能开始使用，无需先注册账号；需要跨设备同步时，再登录或连接 WebDAV。

## 只想直接使用？

打开 [SquiTLe 在线版](https://timeline-planner.blah330903.chatgpt.site) 即可开始，无需安装。

默认情况下，数据只保存在当前浏览器中。你可以随时导出 JSON 备份，也可以在需要时开启同步。
...
无论使用哪种方式，都建议定期导出 JSON 备份。数据格式和冲突处理规则见 [数据与同步说明](docs/data-and-sync.md)。
...
仓库中包含 WebDAV 实现，但 WebDAV 代理目前仍使用官方站点的身份校验。自行部署时，需要替换这部分校验逻辑。目前也没有面向所有平台的一键部署模板。

如果公开发布自定义版本，请更换 SquiTLe 名称和 Logo，避免被误认为官方网站。
...
请保管好自己的数据，无论使用 JSON 导入导出还是云同步，都建议保留自己的备份。

如果遇到数据丢失、无法正常使用等重要问题，可以发邮件到 [tzhu_sh@163.com](mailto:tzhu_sh@163.com)。我平时课程和科研事务比较多，可能无法逐封回复，也无法保证处理时间，但欢迎提出问题、建议和想法。
